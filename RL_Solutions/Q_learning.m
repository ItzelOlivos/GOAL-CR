function [optimal_policy] = Q_learning(Nodes)
    ...
    str = sprintf('../The_size_of_the_network_is_known/Greedy_Continuous_policies/OptimalPolicy_%d_%d', Nodes, 1000);
    load(str)
    
    if T > length(optimal_unrestricted_prob_set)
        added = optimal_unrestricted_prob_set(end)*ones(1, T-length(optimal_unrestricted_prob_set));
        optimal_unrestricted_prob_set = [optimal_unrestricted_prob_set, added];
    end
    ...
    for t = 1:Iterations
        ...
        if mod(t,snap_step) == 0
            ...
            plot(optimal_policy)
            hold on
            plot(optimal_unrestricted_prob_set(1:T))
            hold off
    ...
    str = sprintf('OptimalRL%d.mat', Nodes);
