function [StatisticsDCF, policies] = simulate_FixedOptimal(N, g, execs)
    
    rng default 
        
    DCF = zeros(1, execs);

    p = compute_FixedOptimal(N);    
    ...
    for e = 1:execs
        ...
        for n = 1:N
            ...
            policies(n, 1 + delay:end) = p;
        end

        status = ones(1, N);    % There are N nodes with 1 package each one
        t = 1;    
        while sum(status) > 0
