function [minD, bestFit, bestrsquare, bestrmse, bestgamma, bestmissing] = WeigthExponentialRegression(N, execs)
    ...
    for ptr = 1:length(gamma)
        ...
        str = strcat('@(b,x) min((', num2str(1/N), ').*b(1).^x, 1)');
        fun = str2func(str);    
        start = [.0011];
        ...
        strsave = sprintf('GrowthRule/Candidates/RuleExponential/Results_%d_%d', N, gamma(ptr)*1000);
